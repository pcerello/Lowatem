...
public class JoueurLowatemTest {
    ...
    public void testAjoutDeplDepuis() {
        ...
        NbPointsDeVie nbPv = new NbPointsDeVie(5, 7);
        Case[][] plateau = Utils.plateauDepuisTexte(PLATEAU_NIVEAU4);
        joueur.ajoutDeplDepuis(Coordonnees.depuisCars('a', 'M'), actions, nbPv, plateau);
        // les horizontaux avec la case d'origine
        assertTrue(actions.contient("aMDaM,5,7"));
        assertTrue(actions.contient("aMDaB,5,7"));
        assertTrue(actions.contient("aMDaN,5,7"));
        assertTrue(actions.contient("aMDaG,5,7"));

        // les verticaux
        assertTrue(actions.contient("aMDbM,5,7"));
        assertTrue(actions.contient("aMDcM,5,7"));
        // des mauvais
        assertFalse(actions.contient("aADaM,5,7"));
        assertFalse(actions.contient("aMDaA,5,7"));
        // le bon nombre d'unités
        assertFalse(actions.contient("aMDaN,5,6"));
        // finalement on doit en avoir 1 + 13 + 13
        assertEquals(26, actions.nbActions);
