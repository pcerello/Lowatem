...
public class JoueurLowatem implements IJoueurLowatem {
    ...
    public String[] actionsPossibles(Case[][] plateau, char couleurJoueur, int niveau) {
        ...
        for (int ligne = 0; ligne < plateau.length; ligne++) {
            for (int colonne = 0; colonne < plateau.length; colonne++) {
                if (plateau[ligne][colonne].unitePresente() && couleurJoueur == plateau[ligne][colonne].couleurUnite) {
    ...
    static Coordonnees[] tabCaseVoisine(Coordonnees coord, Case[][] plateau, char couleurJoueur) {
        ...
        if (couleurJoueur == Case.CAR_ROUGE) {
            for (Direction dir : Direction.toutes()) {
                ...
                if (src.estDansPlateau() && plateau[src.ligne][src.colonne].unitePresente() && plateau[src.ligne][src.colonne].couleurUnite == Case.CAR_NOIR) {
            ...
            for (Direction dir : Direction.toutes()) {
                Coordonnees src = coord.suivantes(dir);
                if (src.estDansPlateau() && plateau[src.ligne][src.colonne].unitePresente() && plateau[src.ligne][src.colonne].couleurUnite == Case.CAR_ROUGE) {
                    tabCaseVoisine[i] = src;
                    i++;
                }
            }
    ...
    void ajoutDeplDansDirection(Direction dir, Coordonnees src,
        ...
        while (dst.estDansPlateau() && plateau[dst.ligne][dst.colonne].nature != CAR_EAU) {
    ...
    void ajoutDepl(Coordonnees src, Coordonnees dst, ActionsPossibles actions,
        ...
        if (!plateau[dst.ligne][dst.colonne].unitePresente() && plateau[dst.ligne][dst.colonne].nature != CAR_EAU || src == dst) {
     ...
     */
    static String chaineActionAtt(Coordonnees src, Coordonnees dst, Coordonnees enn, NbPointsDeVie nbPv, Case[][] plateau, char couleurJoueur) {
